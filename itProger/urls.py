...
urlpatterns = [
    ...
    path('user/', authViews.LoginView.as_view(template_name='users/user.html'), name='user'), #Функцию as_view() надо вызывать когда мы вытягиваем класс
...
if settings.DEBUG:
    urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT) #Нужно для того, что бы картики отображалис на локальном сервере
